...
if(!$row['no'])
	alert("상품권번호가 존재하지 않습니다. 확인 후 다시 등록 바랍니다.");

if($row['gr_edate'] < $time_ymd)
	alert("현재 상품권은 사용기간이 만료 되었습니다. \\n\\n만료날짜 : ".$row['gr_edate']);

if($row['gr_sdate'] > $time_ymd)
	alert("현재 상품권은 ".$row['gr_sdate']."일 이후부터 사용 가능하십니다.");

if($row['gi_use'])
	alert("현재 상품권은 이미 등록 된 상태입니다.");
...
alert("정상적으로 처리 되었습니다.", "./gift_list.php");
?>