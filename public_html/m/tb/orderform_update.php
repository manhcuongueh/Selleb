...
if(in_array($_POST['buymethod'],array('B','P'))) { // 무통장, 포인트전액결제
	goto_url("./orderinquiryview.php?odrkey=$odrkey");
} else if($_POST['buymethod']=='K') { // 카카오페이
	goto_url("./orderkakaopay.php?odrkey=$odrkey");
} else {
	if($_POST['settle_case'] == 'kcp') 		
		goto_url("./orderkcp.php?odrkey=$odrkey");
	else if($_POST['settle_case'] == 'ini')
		goto_url("./orderinicis.php?odrkey=$odrkey");
	else if($_POST['settle_case'] == 'all')
		goto_url("./orderallthegate.php?odrkey=$odrkey");
}
?>