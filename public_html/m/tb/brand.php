...
if(check_string($qword, TW_HANGUL) && $qword == 'ㄱ')
	$where[] = " br_name between '가' and '나' ";
else if(check_string($qword, TW_HANGUL) && $qword == 'ㄴ')
	$where[] = " br_name between '나' and '다' ";
else if(check_string($qword, TW_HANGUL) && $qword == 'ㄷ')
	$where[] = " br_name between '다' and '라' ";
else if(check_string($qword, TW_HANGUL) && $qword == 'ㄹ')
	$where[] = " br_name between '라' and '마' ";
else if(check_string($qword, TW_HANGUL) && $qword == 'ㅁ')
	$where[] = " br_name between '마' and '바' ";
else if(check_string($qword, TW_HANGUL) && $qword == 'ㅂ')
	$where[] = " br_name between '바' and '사' ";
else if(check_string($qword, TW_HANGUL) && $qword == 'ㅅ')
	$where[] = " br_name between '사' and '아' ";
else if(check_string($qword, TW_HANGUL) && $qword == 'ㅇ')
	$where[] = " br_name between '아' and '자' ";
else if(check_string($qword, TW_HANGUL) && $qword == 'ㅈ')
	$where[] = " br_name between '자' and '차' ";
else if(check_string($qword, TW_HANGUL) && $qword == 'ㅊ')
	$where[] = " br_name between '차' and '카' ";
else if(check_string($qword, TW_HANGUL) && $qword == 'ㅋ')
	$where[] = " br_name between '카' and '타' ";
else if(check_string($qword, TW_HANGUL) && $qword == 'ㅌ')
	$where[] = " br_name between '타' and '파' ";
else if(check_string($qword, TW_HANGUL) && $qword == 'ㅍ')
	$where[] = " br_name between '파' and '하' ";
else if(check_string($qword, TW_HANGUL) && $qword == 'ㅎ')
	$where[] = " br_name >= '하' ";
else if(check_string($qword, TW_ALPHAUPPER) && $qword)
	$where[] = " SUBSTRING(br_name_eng,1,1) = '$qword' ";
else if(check_string($qword, TW_ALPHALOWER) && $qword)
...
for($i=0; $row=sql_fetch_array($result); $i++) {
	...
	$list[$i]['br_href'] = './brandlist.php?br_id='.$row['br_id'];

	$bimg = TW_DATA_PATH.'/brand/'.$row['br_logo'];
	if(is_file($bimg) && $row['br_logo'])
		$list[$i]['br_logo'] = TW_DATA_URL.'/brand/'.$row['br_logo'];
	else
		$list[$i]['br_logo'] = TW_IMG_URL.'/brlogo_sam.jpg';
}
...
include_once($theme_path.'/brand.skin.php');
