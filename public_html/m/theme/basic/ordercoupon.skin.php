...
<div class="o_coupon">
	...
	<div class="o_cpuse">
		...
		<tbody>
		<tr>
			<td class="mi_dt"><?=get_it_image($row['gs_id'], $gs['simg1'], 60, 60); ?></td>
			<td class="mi_bt">
				<?php echo get_text($gs['gname']); ?><br>
				<span class="strong"><?php echo display_price2($price); ?></span>
			</td>
		</tr>
		<tr id="cp_avail_button_<?php echo $i; ?>">
			<td colspan='2'>
				<button type="button" class="avail_button" onclick="show_coupon('<?php echo $i; ?>');return false;">적용가능 쿠폰선택</button>
			</td>
		</tr>
		</tbody>
		</table>
	...
	<div class="m_post to_box">
		<dl class="to_tline">
			<dt>상품금액 합계</dt>
			<dd><?php echo display_price2($total_price); ?></dd>
			<dt class="point_bg">할인금액 합계</dt>
			<dd class="point_bg"><span id="to_dc_amt">0</span>원</dd>
		</dl>
	</div>
	<div class="tac pg_wrap">
		<button type="button" onclick="cp_submit();return false;" class="btn_medium">쿠폰적용하기</button>
...
<div id="cp_list<?php echo $i; ?>" class="mw" style='display:none;'>
	...
	<div class="fg">
		...
		?>
		<tr>
			<td class=''><input id="ids_shown<?php echo $j; ?>" type="radio" name="use_cp_<?php echo $row['gs_id']; ?>_<?=$row['index_no']?>" value="<?php echo $arr[2]; ?>|<?php echo $arr[5]; ?>|<?php echo $arr[1]; ?>|<?php echo $arr[3]; ?>" class="css-checkbox lrg"><label for="ids_shown<?php echo $j; ?>" class="css-label padr5"><?php echo $arr[2]; ?></label></td>
			<td class="tar"><?php echo $arr[4]; ?></td>
			<td class="tar"><?php echo display_price2($arr[5]); ?></td>
		</tr>
		<?php
			}
		}
		...
		if(!$chk) {
		?>
		<tr><td class="tac" colspan="3" style="height:80px;">사용할 수 있는 쿠폰이 없습니다.</td></tr>
		<?php } ?>
		<tbody>
		</table>

		<div class="mart10 tac">
			<button type="button" onclick="return applycoupon('<?php echo $row['gs_id']; ?>','<?php echo $row['index_no']; ?>','<?php echo $i; ?>');" class="btn_medium wfull">쿠폰적용하기</button>
		</div>
	</div>
</div>
<?php } ?>
...
function cp_submit() {
	...
	if(parseInt(no_comma(total_amt)) < f.sum_dc_amt.value) {
	...
	total_price = parseInt(no_comma(total_amt)) - f.sum_dc_amt.value;

	// 최종 결제금액
	opener.document.buyform.total_amt.value = number_format(String(total_price));
