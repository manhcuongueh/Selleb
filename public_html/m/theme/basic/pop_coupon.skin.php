...
<div class="m_coupon">
	...
	<tr>
		<td class="mi_dt"><?php echo get_it_image($gs['index_no'], $gs['simg1'], 60, 60); ?></td>
		<td class="mi_bt">
			<?php echo get_text($gs['gname']); ?>
			<p class="strong mart5"><?php echo get_price($gs['index_no']); ?></p>
		</td>
	</tr>
	</tbody>
	</table>
	...
	<?php
	if($total_count == 0) {
		echo "<div class=\"sct_noitem\">사용가능한 쿠폰이 없습니다.</div>";
	} else {
	?>
	<table class="tbl_cp mart10">
		...
		for($i=0; $row=sql_fetch_array($result); $i++) {
			...
			// 최대금액
			if($row['cp_low_amt'] > 0) {
				$str .= "<div class='padt5'>&#183; ".display_price2($row['cp_low_amt'])." 이상 구매시</div>";
			}

			// 사용가능대상
			$str .= "<div class='padt5'>&#183; ".$arr_use_part[$row['cp_use_part']]."</div>";

			$s_upd = "<button type=\"button\" onclick=\"post_update('./pop_coupon_update.php', '$cp_id');\" class=\"btn_small\">현재 쿠폰 다운로드</button>";
		?>
		<tr>
			<td class="mi_dw">
				<?php echo $str; ?>
				<div class='mart10'><?php echo $s_upd; ?></div>
			</td>
		</tr>
		<?php } ?>
		</tbody>
	...
	<?php echo pageing($page, $total_page, $total_count, "{$_SERVER['SCRIPT_NAME']}?gs_id=$gs_id&page="); ?>
	<?php } ?>
