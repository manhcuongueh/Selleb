...
$sSiteCode	= $default['de_ipin_id']; // IPIN 서비스 사이트 코드		(NICE신용평가정보에서 발급한 사이트코드)
$sSitePw	= $default['de_ipin_pw']; // IPIN 서비스 사이트 패스워드	(NICE신용평가정보에서 발급한 사이트패스워드)
...
└────────────────────────────────────────────────────────────────────
*/
$sModulePath = $_SERVER['DOCUMENT_ROOT']."/plugin/chekplus/IPIN2Client";
...
if ($sCPRequest == $strCPRequest) {
	echo "<script>location.href='/bbs/register_form.php';</script>";
}
