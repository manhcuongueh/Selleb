...
while($row1 = sql_fetch_array($result)) { // 1차 분류
	...
	$sql2 = " select * from shop_cate where upcate='{$row1['catecode']}' order by list_view asc ";
	...
	while($row2 = sql_fetch_array($result2)) { // 2차 분류
		...
		$sql3 = " select * from shop_cate where upcate='{$row2['catecode']}' order by list_view asc ";
		...
		while($row3 = sql_fetch_array($result3)) { // 3차 분류			
			...
			$sql4 = " select * from shop_cate where upcate='{$row3['catecode']}' order by list_view asc ";
			...
			while($row4 = sql_fetch_array($result4)) { // 4차 분류			
				...
				$sql5 = " select * from shop_cate where upcate='{$row4['catecode']}' order by list_view asc ";
