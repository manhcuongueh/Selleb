<?php
define('_PURENESS_', true);
include_once("./_common.php");

check_demo();

$j_sdate1 = preg_replace('/[^0-9]/', '',$j_sdate);
$j_sdate2 = strtotime($j_sdate1);
$j_sdate3 = $j_sdate2 + 86400;

$j_ddate1 = preg_replace('/[^0-9]/', '',$j_ddate);
$j_ddate2 = strtotime($j_ddate1);
$j_ddate3 = $j_ddate2 + 86400;

$sql_common = " from shop_partner_pay a left join shop_member b on a.mb_id=b.id ";
$sql_search = " where a.mb_id!='admin' and (b.grade between 2 and 6) ";

if($stx && $sfl) {
    switch($sfl) {
        case "mb_id" :
            $sql_search .= " and (a.$sfl like '%$stx%') ";
            break;
		default :
            $sql_search .= " and (b.$sfl like '%$stx%') ";
            break;
    }
}

if($sst)
	$sql_search .= " and b.grade='$sst' ";

if($j_sdate && $j_ddate)
	$sql_search .= " and (b.term_date >= '$j_sdate2' and b.term_date <= '$j_ddate3') ";

if($j_sdate && !$j_ddate)
	$sql_search .= " and (b.term_date >= '$j_sdate2' and b.term_date <= '$j_sdate3') ";

if(!$j_sdate && $j_ddate)
	$sql_search .= " and (b.term_date >= '$j_ddate2' and b.term_date <= '$j_ddate3') ";

if(!$orderby) {
    $filed = "a.total";
    $sod = "desc";
} else {
	$sod = $orderby;
}

$sql_order = " group by a.mb_id order by $filed $sod";

$sql = " select a.* $sql_common $sql_search $sql_order ";
$result = sql_query($sql);
$cnt = @sql_num_rows($result);
if(!$cnt)
	alert("출력할 자료가 없습니다.");

/*================================================================================
php_writeexcel http://www.bettina-attack.de/jonny/view.php/projects/php_writeexcel/
=================================================================================*/

include_once(TW_INC_PATH.'/Excel/php_writeexcel/class.writeexcel_workbook.inc.php');
include_once(TW_INC_PATH.'/Excel/php_writeexcel/class.writeexcel_worksheet.inc.php');

$fname = tempnam(TW_DATA_PATH, "tmp-payexplist.xls");
$workbook = new writeexcel_workbook($fname);
$worksheet = $workbook->addworksheet();

// Put Excel data
$data = array('회원명','아이디','레벨','만료일','현재잔액','분양수수료','광고수수료','판매수수료','관리자적립','전체 누적수수료','전체 지급수수료');
$data = array_map('iconv_euckr', $data);

$col = 0;
foreach($data as $cell) {
    $worksheet->write(0, $col++, $cell);
}

$i = 1;
while($row = sql_fetch_array($result)) {
	$row = array_map('iconv_euckr', $row);	
	
	$mb = get_member($row[mb_id]);
	$mb = array_map('iconv_euckr', $mb);
	
	$sql2 = "select SUM(total) as p_total, 
					SUM(income) as p_income, 
					SUM(outcome)as p_outcome,
					SUM(p_member) as p_member,
					SUM(p_login) as p_login, 
					SUM(p_shop) as p_shop,
					SUM(p_admin) as p_admin
			   from shop_partner_pay
			  where mb_id='$row[mb_id]'";
	$sum = sql_fetch($sql2);

	if($config[p_month]=='y') {
		$h_y = date("Y",$mb[term_date]);
		$h_m = date("m",$mb[term_date]);
		$h_d = date("d",$mb[term_date]);

		$new_hold = mktime(0,0,1,$h_m,$h_d,$h_y);

		$ed = $new_hold - time();
		if($ed > 0) {  $extra_date = round($ed/(60*60*24)); $default_check = 1;}
		else { $exceed_date = round(($ed/(60*60*24))*(-1)); $default_check = 2; }

		if($default_check==2)
			$month = "미납[".$exceed_date."일]"; 
		else
			$month = date('Y/m/d',$mb[term_date]);			
	} else {
		$month = "-";
	}

	$j = 0;
	$worksheet->write($i, $j++, $mb[name]);
	$worksheet->write($i, $j++, ' '.$mb[id]);
	$worksheet->write($i, $j++, $mb[grade]);
	$worksheet->write($i, $j++, iconv_euckr($month));
	$worksheet->write($i, $j++, $sum[p_total]);
	$worksheet->write($i, $j++, $sum[p_member]);
	$worksheet->write($i, $j++, $sum[p_login]);
	$worksheet->write($i, $j++, $sum[p_shop]);
	$worksheet->write($i, $j++, $sum[p_admin]);	
	$worksheet->write($i, $j++, $sum[p_income]);
	$worksheet->write($i, $j++, $sum[p_outcome]);
	$i++;
}

$workbook->close();

$title = iconv_euckr("가맹점수수료");
header("Content-Type: application/x-msexcel; name=\"{$title}-".date("ymd", time()).".xls\"");
header("Content-Disposition: inline; filename=\"{$title}-".date("ymd", time()).".xls\"");
$fh=fopen($fname, "rb");
fpassthru($fh);
unlink($fname);
?>
