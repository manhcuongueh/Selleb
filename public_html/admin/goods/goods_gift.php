...
if($sst) {
    switch($sst) {
		case "g_num" :
            $sql_search .= " and (gr_ty = '0') ";
            break;
        case "g_eng" :
            $sql_search .= " and (gr_ty = '1') ";
            break;
    }
}

if($j_sdate!='' && $j_ddate!='')
{	$sql_search .= " and (gr_sdate >= '$j_sdate' and gr_edate <= '$j_ddate')"; }

if($j_sdate!='' && $j_ddate=='')
{	$sql_search .= " and (gr_sdate >= '$j_sdate' and gr_sdate <= '$j_sdate')"; }

if($j_sdate=='' && $j_ddate!='')
{	$sql_search .= " and (gr_edate >= '$j_ddate' and gr_edate <= '$j_ddate')"; }
	...
	<tr>
		...
		<td>
			<select name="sst">
				...
				<?php echo option_selected('g_num', $sst, '숫자발행'); ?>
				<?php echo option_selected('g_eng', $sst, '영문, 숫자 혼합발행'); ?>
			</select>
		</td>
	</tr>
	</tbody>
...
<form name="fgiftlist" method="post">
...
<div class="tbl_head01">
	...
	<tr>
		...
		<th>NO</th>
		<th><?php echo subject_sort_link('gr_id',$q2); ?>일련번호</a></th>
		<th><?php echo subject_sort_link('gr_sdate',$q2); ?>사용시작</a></th>
		<th><?php echo subject_sort_link('gr_edate',$q2); ?>사용종료</a></th>
		<th><?php echo subject_sort_link('gr_price',$q2); ?>금액</a></th>
		<th>매수</th>
		<th>사용</th>
		<th>발행금액</th>
		<th>사용금액</th>
	...
	for($i=0; $row=sql_fetch_array($result); $i++) {
		...
		$s_cert = "<a href='./goods/goods_gift_serial.php?gr_id=$gr_id' onclick=\"openwindow(this,'pop_serial','700','630','yes');return false\" class=\"btn_small grey\">인증번호</a>";
...
{
	var f = document.fgiftlist;

    if(act == "delete") // 선택삭제
    {
        f.action = './goods/goods_gift_list_delete.php';
        str = "삭제";
    }
    else
        return;
