...
$fname = tempnam(TW_DATA_PATH, "tmp-goodssupplylist.xls");
$workbook = new writeexcel_workbook($fname);
$worksheet = $workbook->addworksheet();

// Put Excel data
$data = array('판매자ID','상품코드','카테고리','상품명','짧은설명','검색키워드','A/S가능여부','모델명','브랜드','과세설정','판매가능지역','판매가능지역 추가설명','원산지','제조사','판매여부','공급가격','시중가격','판매가격','가격대체문구','재고적용타입','재고수량','재고통보수량','최소주문한도','최대주문한도','적립금','판매기간 시작일','판매기간 종료일','구매가능레벨','가격공개','배송비유형','배송비결제','기본배송비','조건배송비','이미지등록방식','소이미지','중이미지1','중이미지2','중이미지3','중이미지4','중이미지5','상세설명','관리자메모');
...
while($row = sql_fetch_array($result)) {
	...
	$j = 0;
	$worksheet->write($i, $j++, ' '.$row['mb_id']); // 판매자ID
