...
if($w == "u" && $bak) {
    $frm_submit .= PHP_EOL.'<a href="./goods.php?code='.$bak.$qstr.'&page='.$page.'" class="btn_large bx-white marl3">목록</a>';
	$frm_submit .= '<a href="goods.php?code=form" class="btn_large bx-red marl3">추가</a>'.PHP_EOL;
}
$frm_submit .= '</div>';
...
<div class="tbl_frm02">
	...
	<tr>
		<th>업체코드</th>
		<td>
			<input type="text" name="mb_id" value="<?php echo $gs['mb_id']; ?>" required itemname="업체코드" class="frm_input w200">
			<a href='supply.php' onclick="openwindow(this,'pop_supply','550','500','no');return false" class="btn_small">업체선택</a>
		</td>
	</tr>
	<tr>
		<th>상품코드</th>
	...
	<tr>
		<th>네이버쇼핑 상품ID</th>
		<td>
			<input type="text" name="ec_mall_pid" value="<?php echo get_text($gs['ec_mall_pid']); ?>" id="ec_mall_pid" class="frm_input w200">
...
<div class="tbl_frm02">
	...
	<tr>
		...
		<td>
			...
			<div id="sit_option_frm"><?php include_once('./goods/goods_option.php'); ?></div>
	...
	<tr>
		...
		<td>
			...
			<div id="sit_option_addfrm"><?php include_once('./goods/goods_spl.php'); ?></div>
	...
	<tr>
		...
		<td>
			...
			<p class="mart5 tac"><a href="./goods/goods_color.php" onclick="openwindow(this,'pop_color','500','640','yes');return false;" class="btn_lsmall red itemicon">상품색상관리</a></p>
		</td>
	</tr>
	</tbody>
	</table>
...
<div class="tbl_frm02">
	...
	<tr>
		<th>수수료 적용타입</th>
		<td>
			<select name="money_type" onChange="chk_money(this.value);">
			<?php echo option_selected('0', $gs['money_type'], '공통설정'); ?>
			<?php echo option_selected('1', $gs['money_type'], '개별설정'); ?>
			</select>
			<a href="javascript:void(0);" onClick="window.open('partner.php?code=money')" class="btn_small grey">설정</a>
		</td>
	</tr>
	<tr>
		<th>수수료 적립단계</th>
		<td>
			<select name="money_yo">
			<?php echo option_selected('%', $gs['money_yo'], '퍼센트로 적립'); ?>
			<?php echo option_selected('$', $gs['money_yo'], '금액으로 적립'); ?>
			</select>
			<input type="text" value='<?php echo $gs['money_dan']; ?>' name="money_dan" onkeyup="chk_rebate(this.value,'<?php echo $gs_id; ?>')" class="frm_input w50"> <span>단계</span>
		</td>
	</tr>
	<tr>
		<th>수수료입력</th>
		<td><span id="chk_rebate_auto"><span></td>
	</tr>
	</tbody>
	</table>
</div>
...
<div class="tbl_frm02">
	...
	<tr>
		...
		<td>
			...
			<a href="javascript:void(0);" onClick="window.open('config.php?code=ship')" class="btn_small grey">설정</a>
...
<div id="sit_compact_fields" class="tbl_frm02 mart7">
	<?php include_once('./goods/goods_info.php'); ?>
</div>
</section>
		...
		<section class="compare_left">
			...
			$(function() {
				$("#btn_search_item").click(function() {
					...
					$("#relation").load(
						gw_admin_url+"/goods/goods_form_relation.php",
...
<script src="<?php echo TW_ADMIN_URL; ?>/js/ajax.js"></script>
...
//수수료 적용
function chk_money(argFun) {
	var f = document.fregform;
	switch (argFun) {
		case "0" :
			f.money_dan.disabled = true;
			f.money_dan.style.backgroundColor = "dddddd";
			f.money_yo.disabled = true;
			eval("chk_rebate_auto").innerHTML = "가맹점관리 > 가맹점 수수료정책 (기본설정 사용중)";
			break;
		case "1" :
			f.money_dan.disabled = false;
			f.money_dan.style.backgroundColor = "";
			f.money_yo.disabled = false;
			eval("chk_rebate_auto").innerHTML = "수수료를 적용할 단계를 입력하세요!";
			break;
	}
}
...
function parseResults() {
	eval("chk_rebate_auto").innerHTML = xmlHttp.responseText;
}

function chk_rebate(no,index){
	eval("chk_rebate_auto").innerHTML = "변경중..";
	doRequestUsingGET(gw_admin_url+"/goods/goods_form_auto.php",no,index);
}
...
chk_money('<?php echo $gs[money_type]; ?>');
chk_img_mod('<?php echo $gs[img_mod]; ?>');
chk_stock('<?php echo $gs[stock_mod]; ?>');
category_first_select();
</script>

<?php if($gs[money_dan]!=0) { ?>
<script>chk_rebate("<?php echo $gs[money_dan]; ?>","<?php echo $gs[index_no]; ?>")</script>
<?php } ?>
