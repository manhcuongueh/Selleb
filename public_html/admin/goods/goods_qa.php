...
<div class="tbl_head01">
	...
	for($i=0; $row=sql_fetch_array($result); $i++) {
		$iq_id = $row['iq_id'];
		$mb = sql_fetch("select index_no from shop_member where id='$row[mb_id]' ");

		$iq_name = "<a href=\"javascript:win_open('/admin/pop_member_main.php?index_no=$mb[index_no]','pop_member','1000','600','yes');\">".$row['iq_name']."</a>";
...
function btn_check(act)
{
    ...
    if(act == "delete") // 선택삭제
    {
        f.action = '/admin/goods/goods_qa_list_delete.php';
        str = "삭제";
    }
    else
        return;
