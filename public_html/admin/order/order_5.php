...
$sql_search = " where (left(gs_se_id,3)='AP-' or gs_se_id = 'admin') and dan = '5' ";
...
$btn_frmline = <<<EOF
<button type="button" onclick="btn_check('dydate')" class="btn_small bx-white">선택수정</button>
<button type="button" onclick="btn_check('decide')" class="btn_small bx-red">구매결정</button>
<a href="./order/order_excel.php?$q1" class="btn_small bx-white"><i class="fa fa-file-excel-o"></i> 엑셀다운로드</a>
EOF;
?>
...
<div class="tbl_head01">
	...
	<tr>
		...
		<th><?php echo subject_sort_link('user_ok',$q2);?>구매결정</a></th>
	</tr>
	</thead>
	<tbody>
	<?php
	for($i=0; $row=sql_fetch_array($result); $i++) {
		...
		if(!$row[user_ok]){
			$s_decide = "<a href=\"javascript:if(confirm('구매완료 처리를 하시겠습니까?')){location.href='./order.php?$q1&od_table=$od_table&mode=decide';}\" class=\"btn_small red\">구매결정</a>";
		} else {
			$s_decide = "<spna class=\"fc_214\">구매완료</span>";
		}
		...
		if($row[gs_se_id] != 'admin') {
			...
			$s_decide .= "<div class=\"mart3\"><a href='pop_member_main.php?index_no=$itmb[index_no]' onclick=\"openwindow(this,'pop_member','1000','600','yes');return false\">".$item[mb_id]."</a><br>(<a href='pop_member_main.php?code=pitem&index_no=$itmb[index_no]' onclick=\"openwindow(this,'pop_seller','1000','600','yes');return false\">".$row[gs_se_id]."</a>)</div>";
		} else {
			$s_decide .= "<div class=\"mart3\">".$row[gs_se_id]."</div>";
		}
	...
	<tr>
		...
		<td rowspan="2"><?php echo $row[pt_id];?></td>
		<td rowspan="2"><?php echo $s_decide;?></td>
	</tr>
	<tr class="rows">
		<td colspan="3">
			<?php echo get_sorts_conf_select("od_delivery[]","","id='dy_{$i}'");?>
			<input type="text" name="od_gonumber[]" value="<?php echo $row[gonumber];?>" class="frm_input w130">
		</td>
	</tr>
	<?php
		...
		echo "<script>document.getElementById('dy_{$i}').value='$row[delivery]';</script>";
	}
	if($i==0)
...
<div class="local_frm02">
	<select name="dan">
		<option value='0'>단계변경</option>
		<option value='6'>반품처리</option>
		<option value='10'>교환처리</option>
	</select>
	<p id="opt_4">
		선택한자료 일괄적용
		<?php echo get_sorts_conf_select('delivery', '', "onchange='chk_delivery(this.value);'");?>
		<span class="marl10">송장번호</span>
		<input type="text" name="gonumber" class="frm_input w120" onkeyup='chk_gonumber(this.value);'>
	</p>
	<button type="button" onclick="btn_check('update')" class="btn_small bx-blue">단계변경</button>
	<?php echo $btn_frmline; ?>
</div>
</form>
...
function btn_check(act)
{
    ...
    if(act == "update") // 선택변경
    {
        f.action = './order/order_update.php';
        str = "변경";
    }
    else if(act == "decide") // 선택 구매결정
    {
        f.action = './order/order_decide.php';
        str = "구매결정";
    }
	else if(act == "dydate") // 선택수정
    {
        f.action = './order/order_update_dy.php';
        str = "수정";
    }
    else
        return;
