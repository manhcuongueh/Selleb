...
$sql_search = " where a.ca_cancel_use='주문취소' and a.ca_it_aff='0' ";
...
if($sst) {
    ...
    switch($sst) {
		case "g_adm" :
            $sql_search .= " (b.gs_se_id = 'admin') ";
            break;
        case "g_use" :
            $sql_search .= " (left(b.gs_se_id,3)='AP-') ";
            break;
		case "ca_g" :
...
<a href="./order/order_cancel_excel.php?$q1" class="btn_lsmall bx-white"><i class="fa fa-file-excel-o"></i> 엑셀다운로드</a>
EOF;
?>
	...
	<tr>
		...
		<td>
			<select name="sfl">
				...
				<?php echo option_selected('b.gs_se_id', $sfl, '판매자ID'); ?>
				<?php echo option_selected('b.pt_id', $sfl, '가맹점ID'); ?>
			</select>
...
<div class="tbl_head01">
	...
	<tr class="<?php echo $bg; ?>">
		...
		<td class="tal">
			...
			// 공급사 정보를 얻는다
			if($row['ca_it_seller'] != 'admin') {
				$item = sql_fetch("select mb_id from shop_seller where sup_code = TRIM('$row[ca_it_seller]')");
				$itmb = sql_fetch("select index_no from shop_member where id = TRIM('$item[mb_id]')");
				$row['ca_it_seller'] = "<a href='pop_member_main.php?index_no=$itmb[index_no]' onclick=\"openwindow(this,'pop_member','1000','600','yes');return false\">".$item['mb_id']."</a>&nbsp;(<a href='pop_member_main.php?code=pitem&index_no=$itmb[index_no]' onclick=\"openwindow(this,'pop_seller','1000','600','yes');return false\">".$row['ca_it_seller']."</a>)";
			}
			?>
		</td>
...
function btn_check(act)
{
    ...
    if(act == "update") // 선택수정
    {
		f.action = './order/order_cancel_update.php';
        str = "수정";
    }
	else if(act == "ca_yn") // 선택승인
    {
        f.action = './order/order_cancel_yn.php';
        str = "취소";
    }
	else if(act == "delete") // 선택삭제
    {
        f.action = './order/order_cancel_delete.php';
        str = "삭제";
    }
    else
        return;
