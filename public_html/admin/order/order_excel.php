...
$sql_intere = " where (left(gs_se_id,3)='AP-' or gs_se_id = 'admin') ";

switch($code){
	case 'today':
		$sql_search = " $sql_intere and orderdate_s = '$time_ymd' and dan	!= '0' ";
		break;
	case 'whole':
		$sql_search = " $sql_intere and dan != '0' ";
		break;
	default :
		$sql_search = " $sql_intere and dan = '$code' ";
		break;
}
...
// Put Excel data
$data = array('가맹점','판매자','주문번호','일련번호','상품명','옵션','판매정가','주문수량','쿠폰할인','회원할인','적립금결제','배송비','실결제금액','총계','공급가','결제방법','현황','주문자명','수취인명','수취인연락처','수취인핸드폰','수취인우편번호','수취인주소','주문시전달사항','배송업체','송장번호','입금자명','입금날짜','주문날짜','사업자등록번호','사업자상호명','대표명','사업장소재지','업태','종목','매입계산서 요청','현금영수증 사업자지출용','현금영수증 개인소득용');
...
while($row = sql_fetch_array($result)) {
	...
	for($g=0; $ct=sql_fetch_array($res); $g++) {
		...
		// 합계금액 계산
		$sql = " select SUM(IF(io_type = 1, (io_price * ct_qty),((io_price + $gs[daccount]) * ct_qty))) as it_damt,
						SUM(IF(io_type = 1, (io_price * ct_qty),((io_price + ct_price) * ct_qty))) as ct_amt,
						SUM(IF(io_type = 1, (io_price * ct_qty),((io_price + $gs[account]) * ct_qty))) as it_amt,
	...
	$ct_amt = (int)$sum['ct_amt'];
	$it_amt = (int)$sum['it_amt'];
	$it_damt = (int)$sum['it_damt'];
	$qty = (int)$sum['qty'];

	$sale_amt = 0;
	if($it_amt > $ct_amt) {
		$sale_amt = $it_amt - $ct_amt;
	}
	...
	$worksheet->write($i, $j++, $gs[gname]);	
	$worksheet->write($i, $j++, iconv_euckr($it_name));
	$worksheet->write($i, $j++, $it_amt);
	$worksheet->write($i, $j++, $qty);	
	$worksheet->write($i, $j++, $row[dc_exp_amt]);
	$worksheet->write($i, $j++, $sale_amt);	
