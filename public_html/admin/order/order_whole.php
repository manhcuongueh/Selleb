...
$sql_search = " where (left(gs_se_id,3)='AP-' or gs_se_id = 'admin') and dan != '0' ";
...
<div class="tbl_head01">
	...
	for($i=0; $row=sql_fetch_array($result); $i++) {
		...
		if($row['delivery']) {
			...
			if(!$delivery[1]) {
				...
				$baesong .= "<a href='".$delivery[1].$row['gonumber']."' onclick=\"openwindow(this,'pop_delivery','600','650','yes');return false\" class=\"btn_ssmall bx-white\">";
			}
			$baesong .= "배송추적</a>";
		}
	...
	<?php
		if($max[max_idx] == $od_table) {
			...
			echo "<td colspan='3'>".$baesong.'&nbsp;&nbsp;'.$delivery[0].'&nbsp;&nbsp;'.$row[gonumber]."</td>";
...
function btn_check(act)
{
    ...
    if(act == "delete") // 선택삭제
    {
        f.action = './order/order_whole_delete.php';
        str = "삭제";
    }
    else
        return;
