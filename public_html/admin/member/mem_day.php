...
$mb1 = sql_fetch("select count(*) as cnt $sql_common where id <> 'admin'");
$mb2 = sql_fetch("select count(*) as cnt $sql_common where grade between '7' and '9' ");
$mb3 = sql_fetch("select count(*) as cnt $sql_common where grade between '2' and '6' ");
?>
...
<div class="local_frm01">
	...
	<span class="fr">
		...
		for($i=1;$i<=12;$i++) {
			$k = sprintf('%02d',$i);
			echo option_selected($k, $month, $k.'월');
		}
		?>
		</select>
...
<div class="tbl_head01">
	...
	for($i=1; $i<=31; $i++) {
		...
		$search_d = $year."-".sprintf('%02d',$month)."-".sprintf('%02d',$i);

		$row1 = sql_fetch("select count(*) as cnt $sql_common where left(reg_time,10)='$search_d' and id!='admin'");
		$row2 = sql_fetch("select count(*) as cnt $sql_common where left(reg_time,10)='$search_d' and grade between '7' and '9' ");
		$row3 = sql_fetch("select count(*) as cnt $sql_common where left(reg_time,10)='$search_d' and grade between '2' and '6' ");

		if($row1['cnt'] == 0 || $mb1['cnt'] == 0)
			$per = 0;
		else
