...
<div class="tbl_frm01">
	...
	<tr>
		<th>아이코드 아이디</th>
		<td>
			<input type="text" name="cf_icode_id" id="cf_icode_id" value="<?php echo $sms['cf_icode_id']; ?>" class="frm_input w200" required itemname="아이코드 아이디" maxlength="10">
			<a href="http://icodekorea.com/res/join_company_fix_a.php?sellid=gnd" target="_blank" class="btn_small grey">아이코드 회원가입</a>
			<?php echo help('아이코드에서 가입하신 회원아이디를 입력합니다.<br>gnd_ 를 포함해 최대 10자를 넘지 않아야 합니다.<br>gnd_ 는 아이코드에서 회원가입시 자동으로 삽입되는 부분입니다.'); ?>
		</td>
	</tr>
	<tr>
		<th>아이코드 패스워드</th>
		<td>
			<input type="password" name="cf_icode_pw" id="cf_icode_pw" value="<?php echo $sms['cf_icode_pw']; ?>" class="frm_input w200" required itemname="아이코드 패스워드" maxlength="10">
			<?php echo help('아이코드에서 가입하신 비밀번호를 입력합니다.<br>비밀번호는 최대 10자를 넘지 않아야 합니다.'); ?>
		</td>
	</tr>
	<tr>
		<th>발신 전화번호</th>
		<td>
			<input type="text" name="cf_sms_recall" id="cf_sms_recall" value="<?php echo $sms['cf_sms_recall']?>" class="frm_input w200" required itemname="발신 전화번호">
...
<tr>
	<td>
		...
		<textarea id="cf_cont<?php echo $i; ?>" name="cf_cont<?php echo $i; ?>" onkeyup="byte_check('cf_cont<?php echo $i; ?>', 'byte<?php echo $i; ?>', 'max_byte<?php echo $i; ?>');"><?php echo $sms["cf_cont{$i}"]; ?></textarea>
	...
	<td class="vat td_label">
		...
		<p>
			<input id="cf_mb_use<?php echo $i; ?>" type="checkbox" name="cf_mb_use<?php echo $i; ?>" value="1" <?php echo get_checked($sms["cf_mb_use{$i}"], "1"); ?>> <label for="cf_mb_use<?php echo $i; ?>">고객</label>
			<input id="cf_ad_use<?php echo $i; ?>" type="checkbox" name="cf_ad_use<?php echo $i; ?>" value="1" <?php echo get_checked($sms["cf_ad_use{$i}"], "1"); ?>> <label for="cf_ad_use<?php echo $i; ?>">관리자</label>
			<input id="cf_re_use<?php echo $i; ?>" type="checkbox" name="cf_re_use<?php echo $i; ?>" value="1" <?php echo get_checked($sms["cf_re_use{$i}"], "1"); ?>> <label for="cf_re_use<?php echo $i; ?>">가맹점</label>
			<?php if($i > 1) { ?>
			<input id="cf_sr_use<?php echo $i; ?>" type="checkbox" name="cf_sr_use<?php echo $i; ?>" value="1" <?php echo get_checked($sms["cf_sr_use{$i}"], "1"); ?>> <label for="cf_sr_use<?php echo $i; ?>">공급사</label>
			<?php } ?>
		</p>
	...
	<td>
		<div class="scf_sms_img">
		<textarea id="cf_cont<?php echo $k; ?>" name="cf_cont<?php echo $k; ?>" onkeyup="byte_check('cf_cont<?php echo $k; ?>', 'byte<?php echo $k; ?>', 'max_byte<?php echo $k; ?>');"><?php echo $sms["cf_cont{$k}"]; ?></textarea>
	...
	<td class="vat td_label">
		...
		<p>
			<input id="cf_mb_use<?php echo $k; ?>" type="checkbox" name="cf_mb_use<?php echo $k; ?>" value="1" <?php echo get_checked($sms["cf_mb_use{$k}"], "1"); ?>> <label for="cf_mb_use<?php echo $k; ?>">고객</label>
			<input id="cf_ad_use<?php echo $k; ?>" type="checkbox" name="cf_ad_use<?php echo $k; ?>" value="1" <?php echo get_checked($sms["cf_ad_use{$k}"], "1"); ?>> <label for="cf_ad_use<?php echo $k; ?>">관리자</label>
			<input id="cf_re_use<?php echo $k; ?>" type="checkbox" name="cf_re_use<?php echo $k; ?>" value="1" <?php echo get_checked($sms["cf_re_use{$k}"], "1"); ?>> <label for="cf_re_use<?php echo $k; ?>">가맹점</label>
			<input id="cf_sr_use<?php echo $k; ?>" type="checkbox" name="cf_sr_use<?php echo $k; ?>" value="1" <?php echo get_checked($sms["cf_sr_use{$k}"], "1"); ?>> <label for="cf_sr_use<?php echo $k; ?>">공급사</label>
		</p>
