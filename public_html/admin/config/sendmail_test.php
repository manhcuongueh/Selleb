...
if(isset($_POST['email'])) {
    ...
    for($i=0; $i<count($email); $i++)
        mailer($config['company_name'], $super['email'], trim($email[$i]), '[메일검사] 제목', '<span style="font-size:9pt;">[메일검사] 내용<p>이 내용이 제대로 보인다면 보내는 메일 서버에는 이상이 없는것입니다.<p>'.$time_ymdhis.'<p>이 메일 주소로는 회신되지 않습니다.</span>', 1);
...
<div class="tbl_frm01">
	...
	<tr>
		...
		<td>				
			<input type="text" name="email" value="<?php echo $member['email']; ?>" id="email" required class="required email frm_input" size="60">
