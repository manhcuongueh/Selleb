...
$upl_dir = TW_DATA_PATH."/board/boardimg";
...
if($w == "") {
	...
	include "./board_sql.php";
	@sql_query($board_table);
	@sql_query($board_tail_table);
	$path = TW_DATA_PATH."/board/".$bo_table;
	exec("mkdir  $path");
 	exec("chmod 707 $path");

	goto_url("../config.php?code=board_form&w=u&bo_table=$bo_table");
	...
	goto_url("../config.php?code=board_form&w=u&bo_table=$bo_table$qstr&page=$page");
}
?>