...
if($w == "reg") {
	...
	goto_url("config.php?code=keyword&page=$page");
}

$sql_common = " from shop_keyword ";
$sql_search = " where pt_id='admin' ";
...
<div class="tbl_frm02">
	...
	<tr>
		<th>검색어등록</th>
		<td><input type="text" name="keyword" class="frm_input" required itemname="검색어" size="40"></td>
	</tr>
	</tbody>
	</table>
</div>
...
function fregform_submit(f) {
	f.action = "config.php?code=keyword";
    return true;
}
...
function btn_check(act)
{
    ...
    if(act == "update") // 선택수정
    {
        f.action = './config/keyword_update.php';
        str = "수정";
    }
    else if(act == "delete") // 선택삭제
    {
        f.action = './config/keyword_delete.php';
        str = "삭제";
    }
    else
        return;
