...
if(get_cookie('ck_visit_ip') != $_SERVER['REMOTE_ADDR'])
{
    ...
    $sql = " insert shop_visit ( vi_id, mb_id, vi_ip, vi_date, vi_time, vi_referer, vi_agent ) values ( '{$vi_id}', '{$mk['id']}', '{$remote_addr}', '{$time_ymd}', '{$time_his}', '{$referer}', '{$user_agent}' ) ";
    ...
    if($result) {
        ...
        if($tmp_cnt) {
			...
			$sql = " insert shop_visit_sum (vs_count, vs_date, mb_id) values (1, '{$time_ymd}', '{$mk['id']}') ";
			sql_query($sql);
		}
        ...
        $sql = " select vs_count as cnt 
				   from shop_visit_sum
				   where vs_date = DATE_SUB('{$time_ymd}', INTERVAL 1 DAY)
				     and mb_id = '{$mk['id']}' ";
        $row = sql_fetch($sql);
        ...
        // 전체
        $sql = " select sum(vs_count) as total from shop_visit_sum where mb_id = '{$mk['id']}' ";
        $row = sql_fetch($sql);
