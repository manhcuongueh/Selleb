...
class upload_files
{
	...
	function upload(&$file)
	{
		...
		if(!is_dir($this->upl_dir)) {
			@mkdir($this->upl_dir, TW_DIR_PERMISSION);
			@chmod($this->upl_dir, TW_DIR_PERMISSION);
		}
		...
		if(move_uploaded_file($file['tmp_name'], $this->upl_dir.'/'.$filename)) {
			@chmod($this->upl_dir.'/'.$filename, TW_FILE_PERMISSION);
...
function get_view_thumbnail($contents, $thumb_width=0)
{
    ...
    for($i=0; $i<count($matches[1]); $i++) {
        ...
        if(is_file($srcfile)) {
            ...
            if($width) {
                $thumb_tag = '<img src="'.TW_URL.str_replace($filename, $thumb_file, $data_path).'" alt="'.$alt.'" width="'.$width.'" height="'.$height.'"/>';
            } else {
                $thumb_tag = '<img src="'.TW_URL.str_replace($filename, $thumb_file, $data_path).'" alt="'.$alt.'"/>';
            }

            // $img_tag에 editor 경로가 있으면 원본보기 링크 추가
            $img_tag = $matches[0][$i];
            if(strpos($img_tag, 'data/editor') && preg_match("/\.(gif|jpg|jpeg|png)$/i", $filename)) {
                $imgurl = str_replace(TW_URL, "", $src);
                $thumb_tag = '<a href="'.TW_INC_URL.'/view_image.php?fn='.urlencode($imgurl).'" target="_blank">'.$thumb_tag.'</a>';
            }
...
function thumbnail($filename, $source_path, $target_path, $thumb_width, $thumb_height, $is_create, $is_crop=false, $crop_mode='center', $is_sharpen=false, $um_value='80/0.5/3', $is_srand=false)
{
    ...
    if(!is_dir($target_path)) {
        @mkdir($target_path, TW_DIR_PERMISSION);
        @chmod($target_path, TW_DIR_PERMISSION);
    }
	...
	if($is_srand) {
		srand((double)microtime()*1000000);
		$thumb_filename .= '_'.mt_rand(10000,99999);
	} else {
		$thumb_filename  = "thumb-{$thumb_filename}_{$thumb_width}x{$thumb_height}";
	}
    ...
    if($size[2] == 1) {
        ...
        if(!defined('TW_THUMB_PNG_COMPRESS'))
            $png_compress = 5;
        else
            $png_compress = TW_THUMB_PNG_COMPRESS;

        imagepng($dst, $thumb_file, $png_compress);
    } else {
        if(!defined('TW_THUMB_JPG_QUALITY'))
            $jpg_quality = 90;
        else
            $jpg_quality = TW_THUMB_JPG_QUALITY;

        imagejpeg($dst, $thumb_file, $jpg_quality);
    }

    chmod($thumb_file, TW_FILE_PERMISSION); // 추후 삭제를 위하여 파일모드 변경
