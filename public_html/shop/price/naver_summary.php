...
$sql_search = " and SUBSTRING(a.update_time, 1, 10) = '{$time_ymd}' ";
...
for($i=0; $row=sql_fetch_array($result); $i++)
{
    ...
    // 배송비계산
    $delivery = get_sendcost_amt2($row['index_no'], $row['account']);

    // 상품이미지
    $img_url = get_it_image_url($row['index_no'], $row['simg2'], 400, 400);

	$item_link = TW_SHOP_URL.'/view.php?index_no='.$row['index_no'];
    
    // 상태
    $class = 'U';
    $stock_qty = get_it_stock_qty($row['index_no']);

    if(substr($row['reg_time'], 0, 10) == $time_ymd && $row['update_time'] >= $row['reg_time'])
        $class = 'I';

    if($row['isopen'] > 1 || $stock_qty < 0)
        $class = 'D';

    echo "\n{$row['index_no']}{$tab}{$row['gname']}{$tab}{$row['account']}{$tab}{$item_link}{$tab}{$img_url}{$tab}{$cate1}{$tab}{$cate2}{$tab}{$cate3}{$tab}{$row['model']}{$tab}{$row['brand_nm']}{$tab}{$row['maker']}{$tab}{$row['origin']}{$tab}{$row['gpoint']}{$tab}{$delivery}{$tab}{$class}{$tab}{$row['update_time']}";
}
