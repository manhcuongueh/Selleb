...
if(!is_admin() && !$default['cf_card_test_yn'] && $default['de_naverpay_test']) {
    if($default['de_naverpay_mb_id'] && (!$mb_yes || $member['id'] != $default['de_naverpay_mb_id']))
        return;
}
...
if($default['de_naverpay_test'] || $default['cf_card_test_yn']) {
...
function buy_nc(url)
{
    ...
    if( check ) {
        ...
        //f.action = "'.TW_SHOP_URL.'/naverpay/naverpay_order.php";
        //f.target = "win_buy_nc";
        //f.submit();
        //return false;

        $.ajax({
            url : "'.TW_SHOP_URL.'/naverpay/naverpay_order.php",
            type : "POST",
            data : $(f).serialize(),
            async : false,
            cache : false,
...
function wishlist_nc(url)
{
    ...
    f.action = "'.TW_SHOP_URL.'/naverpay/naverpay_wish.php";
    f.submit();
