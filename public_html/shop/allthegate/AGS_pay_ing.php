...
/*공통사용*/
$agspay->SetValue("AgsPayHome", ROOT_AGS); //올더게이트 결제설치 디렉토리 (상점에 맞게 수정)
...
$agspay->SetValue("ProdNm",trim(iconv_euckr($_POST["ProdNm"]))); //상품명
...
/*신용카드사용*/
$agspay->SetValue("OrdNm",trim(iconv_euckr($_POST["OrdNm"]))); //주문자명
$agspay->SetValue("OrdPhone",trim($_POST["OrdPhone"]));		//주문자연락처
$agspay->SetValue("OrdAddr",trim(iconv_euckr($_POST["OrdAddr"]))); //주문자주소 가상계좌추가
$agspay->SetValue("RcpNm",trim(iconv_euckr($_POST["RcpNm"]))); //수신자명
$agspay->SetValue("RcpPhone",trim($_POST["RcpPhone"]));		//수신자연락처
$agspay->SetValue("DlvAddr",trim(iconv_euckr($_POST["DlvAddr"]))); //배송지주소
$agspay->SetValue("Remark",trim(iconv_euckr($_POST["Remark"]))); //비고
...
/*계좌이체사용*/
$agspay->SetValue("ICHE_OUTBANKNAME",trim(iconv_euckr($_POST["ICHE_OUTBANKNAME"]))); //이체은행명
$agspay->SetValue("ICHE_OUTACCTNO",trim($_POST["ICHE_OUTACCTNO"])); //이체계좌번호
$agspay->SetValue("ICHE_OUTBANKMASTER",trim(iconv_euckr($_POST["ICHE_OUTBANKMASTER"]))); //이체계좌소유주
...
{ 
	...
	$cash['tid']			= $agspay->GetResult("rDealNo");	// 거래번호
	$cash['card_code']		= $agspay->GetResult("rApprNo");	// 승인번호
	$cash['card_bankcode']	= $agspay->GetResult("rCardCd");	// 카드사코드
	$cash['appldate']		= $agspay->GetResult("rApprTm");	// 승인시각
	$cash['applnum']		= $agspay->GetResult("rBusiCd");	// 전문코드
	$cash['acct_bankcode']	= $agspay->GetResult("rStoreId");	// 업체아이디								
	$cash['vact_num']		= $agspay->GetResult("rVirNo");		// 입금계좌번호(가상계좌번호)
	$cash['vact_bankcode']	= $agspay->GetResult("VIRTUAL_CENTERCD");	// 입금은행코드
	$cash['vact_date']		= $agspay->GetResult("rApprTm");	// 승인일자
	$cash['vact_inputname']	= '';								// 송금자명
	$cash['vact_name']		= '';								// 입금할 계좌 예금주
	$cash['hpp_num']		= $agspay->GetResult("rHP_HANDPHONE");	// 핸드폰결제일
	$cash['ss_pg_id']		= '';	// 개별결제 검사 (가맹점 ID)
	...
	$sql = "update shop_cart set ct_select='1' where odrkey='$rOrdNo'";
	sql_query($sql, FALSE);	
...
{
	// 결제실패에 따른 상점처리부분
	$msg = "[".$agspay->GetResult("rSuccYn")."]". iconv_utf8($agspay->GetResult("rResMsg"));
}

$gw_head_title = '올더게이트 지불 결과';
...
<?php
if ($agspay->GetResult("rSuccYn") == 'y') {
	alert($msg, TW_SHOP_URL."/orderinquiryview.php?odrkey=$rOrdNo");
} else {
	alert($msg);
}
