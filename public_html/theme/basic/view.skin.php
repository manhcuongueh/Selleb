...
<div class="vi_info">
	<div class="vi_img_bx" style="width:<?php echo $default['cf_item_medium_wpx']; ?>px">
		<?php if($is_social_ing) { include_once($theme_path.'/time.skin.php'); } ?>
		<?php if($is_social_end) { ?><div class="t_social"><?php echo $is_social_txt; ?></div><?php } ?>

		<div class="bimg">
		<?php
		if($gs['simg2'])
			echo get_it_image($index_no, $gs['simg2'], $default['cf_item_medium_wpx'], $default['cf_item_medium_hpx'], "id='big'");
		else
			echo "<img src='".TW_IMG_URL."/sub/noimage_q2.gif' width='".$default['cf_item_medium_wpx']."' height='".$default['cf_item_medium_hpx']."' id='big'>";
		?>
		</div>
		<div class="simg_li">
			<ul>
				...
				for($i=2; $i<=6; $i++) {
					...
					$thumbnails = get_it_image_url($index_no, $it_image, $default['cf_item_medium_wpx'], $default['cf_item_medium_hpx']);
				?>
	...
	<div class="vi_txt_bx">
		<h2 class="tit">
			<?php echo $gs['gname']; ?>
			<?php if(is_admin()) { ?><a href="<?php echo TW_ADMIN_URL; ?>/goods.php?code=form&w=u&gs_id=<?php echo $index_no; ?>" target="_blank" class="btn_small red">수정</a><?php } ?>
			<?php if($gs['explan']) { ?>
		...
		<div class="price_bx">
			...
			<dl>
				<dt>시중가격</dt>
				<dd class="f_price"><?php echo display_price2($gs['saccount']); ?></dd>
			</dl>
			<?php } ?>
			<dl>
		...
		<div class="vi_txt_li">
			...
			<dl>
				<dt>고객상품평</dt>
				<dd>상품평 : <?php echo $item_use_count; ?>건, 평점 : <img src="<?php echo TW_IMG_URL; ?>/sub/view_score_<?php echo $star_score; ?>.gif"></dd>
			</dl>
			<dl>
				<dt>상품URL 소셜 공유</dt>
...
<div class="mart50">
	...
	<div class="tbl_frm01 mart15">
		...
		<tr>
			<th>상품코드</th>
			<td class='padl10'><?php echo $gs['gcode']; ?></td>
			<th class="bl">부가세, 면세여부</th>
			<td class='padl10'><?php echo ($gs['notax'])?"과세상품":"면세상품"; ?></td>
		</tr>
		<tr>
			<th>A/S문의</th>
			<td class='padl10'><?php echo ($gs['repair'])?$gs['repair']:"제조사 A/S문의"; ?></td>
			<th class="bl">증빙서류발급</th>
			<td class='padl10'><?php echo ($gs['notax'])?"세금계산서, 현금영수증 발급가능":"세금계산서, 현금영수증 발급불가능"; ?></td>
		</tr>
		</table>
	</div>
	...
	?>
	<table class="wfull mart20 marb30">
		<tr height="30">
			<td><strong class="fs14">※ 전자상거래 등에서의 상품정보제공고시</strong></td>
		</tr>
		<tr>
			<td height="1" bgcolor="#635e5e"></td>
		</tr>
		<tr>
			<td>
				<div class="tbl_frm01">
...
<div class="mart50">
	...
	<div class="mart15 vi_qa">
		<?php
		include_once($theme_path.'/view_qa.skin.php');
		?>
	</div>
</div>
