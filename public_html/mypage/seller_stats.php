...
$orderdate = preg_replace("/([0-9]{4})([0-9]{2})/", "\\1-\\2", $year.$month);

$sql = " select count(*) as cnt,
				sum(account + del_account) as od_price
		   from shop_order 
		   where dan between '1' and '5' 
		     and left(orderdate_s,7) = '$orderdate'
			 and gs_se_id = '$seller[sup_code]' ";
$row = sql_fetch($sql);
...
<div class="local_frm01">
	<p class="fl mart3">
		<b><?php echo $orderdate;?></b>월 , 전체 : <b class="fc_197"><?php echo number_format($tot_count);?></b> 건 조회 , 전체주문금액 : <b class="fc_197"><?php echo number_format($tot_price);?></b> 원
	</p>
	<p class="fr">
		<select name="year">
			<?php
