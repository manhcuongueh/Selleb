...
if($w == "u" && $bak) {
	...
	$frm_submit .= '<a href="page.php?code=partner_goods_form" class="btn_large bx-red marl3">추가</a>'.PHP_EOL;
}
$frm_submit .= '</div>';
	...
	<tr>
		<td>
			...
			while($row = sql_fetch_array($res)) {
				echo "<option value='$row[gcate]'>".get_move_aff($row['gcate'])."</option>\n";
			}
			?>
			</select>
...
function fregform_submit(f) {
	...
	f.action = "./partner_goods_form_update.php";
    return true;
}
