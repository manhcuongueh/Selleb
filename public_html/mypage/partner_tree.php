...
if(!$type)
	...
	function mb_tree($mb_recommend,$level,$line_array,$pre_cnt=0){
		...
		if($r['grade']>1  || $mb_recommend =='admin'){
			echo("<img src='/admin/img/$img_name.gif' width='15' height='15' align='absmiddle'>&nbsp;<b><font color='$t_color'>($t_cnt)</font>&nbsp;{$r[name]}{$line}{$mb_recommend}{$line}<font color='ed8e06'>{$mb_grade}</font>{$line}휴대폰:<font color='939393'>{$r[cellphone]}</font>{$line}가입:<font color='939393'>".substr($r['reg_time'],0,10)."</font>{$line}로그인수:<font color='939393'>{$r[login_sum]}</font></b>");
		}
		...
		<div style='width:100%;background:url(/admin/img/les/Code_line.gif);overflow-x:auto; overflow-y:auto;'>
			...
			<tr>
				<td style="padding:0 3px;">
					<?php
					$line_array=array();
					mb_tree($member['id'],0,$line_array);
					?>
				</td>
			</tr>
			</table>
