		...
		if($cash['tid']) {
			...
			if($cash['tpg'] == 'kcp') {
				...
				if(in_array($od['buymethod'], array('C','R','ER'))) {
					...
					$default['cf_card_test_yn']	= $partner['cf_card_test_yn']; // 결제시스템 방식
					$default['cf_escrow_yn'] = $partner['cf_escrow_yn']; // Escrow 사용여부
					$default['cf_kcp_id'] = $partner['cf_kcp_id']; // KCP PG ID
					$default['cf_kcp_key'] = $partner['cf_kcp_key']; // KCP KEY
					$default['cf_nm_pg'] = $partner['cf_nm_pg']; // 상점명
					$default['cf_kcp_quota'] = $partner['cf_kcp_quota']; // 최대 할부 개월
					$default['cf_kcp_noint_yn']	= $partner['cf_kcp_noint_yn']; // 무이자 사용 여부 	
					$default['cf_kcp_noint_mt']	= $partner['cf_kcp_noint_mt']; // 무이자 사용 여부
					$default['cf_kcp_tax_yn'] = $partner['cf_kcp_tax_yn']; //현금 영수증 발급
			...
			if($cash['tpg'] == 'inicis' && $_POST['ca_type'][$k] == '일반취소') {	
				...
				// 실결제시
					...
					if($cash['escw_yn'] == 'Y') {
						// 에스크로결제 테스트
						$_POST['mid'] = $partner['cf_inicis_escrow_id'];
					}
					else {
						// 일반결제 테스트
						$_POST['mid'] = $partner['cf_inicis_id'];
					}
				}
			...
			if($cash['tpg'] == 'kakaopay') {
				...
				$default['cf_card_test_yn'] = $partner['cf_card_test_yn']; // 결제시스템 방식
				$default['de_kakaopay_mid'] = $partner['de_kakaopay_mid']; // 상점MID  
				$default['de_kakaopay_key'] = $partner['de_kakaopay_key']; // 상점키
				$default['de_kakaopay_enckey'] = $partner['de_kakaopay_enckey']; // EncKey
				$default['de_kakaopay_hashkey'] = $partner['de_kakaopay_hashkey']; // HashKey  
				$default['de_kakaopay_cancelpwd'] = $partner['de_kakaopay_cancelpwd']; // 결제취소 P/W
