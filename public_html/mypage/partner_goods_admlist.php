...
$sql_common = " from shop_goods a left join shop_goods_cate b on ( a.index_no = b.gs_id ) left join {$target_table} c on ( b.gcate = c.catecode ) ";

$sql_search = " where a.use_aff = '0' and a.shop_state = '0' and c.p_hide = '0' ";
...
if(isset($q_state) && is_numeric($q_state)) {
	if($q_state)
		$sql_search .= " and (find_in_set('$member[id]', a.use_hide) >= 1) ";
	else
		$sql_search .= " and (find_in_set('$member[id]', a.use_hide) = 0) ";
}
...
$adm_category_yes = true;
include_once(TW_INC_PATH."/categoryinfo.lib.php");
include_once(TW_PLUGIN_PATH.'/jquery-ui/datepicker.php');

$btn_frmline = <<<EOF
<button type="button" onclick="btn_check('hide');" class="btn_lsmall bx-red">선택상품숨김</button>	
<button type="button" onclick="btn_check('show');" class="btn_lsmall bx-blue">선택상품노출</button>
EOF;
...
<div class="tbl_frm01">
	...
	<tr>
		...
		<td colspan="3">
			...
			<?php echo radio_checked('q_state', $q_state, '1', '감춤'); ?>
			<?php echo radio_checked('q_state', $q_state, '0', '노출'); ?>
		</td>
	</tr>
	</tbody>
	</table>
...
<div class="tbl_head02">
	...
	<tr>
		...
		<th colspan="<?php echo $t_colspan; ?>" class="th_bg">가격정보</th>
	</tr>
	<tr class="rows">
		<th>노출상태</th>
		<th>카테고리</th>
	...
	for($i=0; $row=sql_fetch_array($result); $i++) {		
		...
		$count_yes = false;
		if(in_array($member['id'], explode(",", $row['use_hide']))) {
			$count_yes = true;
		}
	...
	<tr class="<?php echo $bg; ?>">
		...
		<?php if($config['p_payment_yes']) { ?>
		<td rowspan="2" class="tar fc_00f"><?php echo number_format(get_payment($gs_id)); ?></td>
		<?php } ?>
	</tr>
	<tr class="<?php echo $bg; ?>">
		<td><?php echo $count_yes ? '<span class="txt_fail">감춤</span>':'<span class="fc_00f">노출</span>'; ?></td>
...
function btn_check(act)
{
    ...
    if(act == "hide") // 선택숨김
    {
        f.action = './partner_goods_admlist_update.php?w=hide';
        str = "숨김";
    }
    else if(act == "show") // 선택노출
    {
        f.action = './partner_goods_admlist_update.php?w=show';
        str = "노출";
    }
    else
        return;
