...
$sql_search = " where gs_se_id = '$member[id]' ";
...
$btn_frmline = '';
if(in_array($set, array('1','2','3'))) {
	$btn_frmline .= "<button type=\"button\" onclick=\"btn_check('delete')\" class=\"btn_small bx-white\">선택삭제</button> ";
}
$btn_frmline .= <<<EOF
<button type="button" onclick="btn_check('dydate')" class="btn_small bx-white">선택수정</button>
<a href="./partner_odr_excel.php?$q1" class="btn_small bx-white"><i class="fa fa-file-excel-o"></i> 엑셀다운로드</a>
EOF;
?>
	...
	<tr>
		...
		<td>
			<select name="sst">
				...
				<?php if(in_array($set, array('today','whole'))) { ?>
				<optgroup id="optg4">
					<?php
					for($i=1; $i<=10; $i++) {
						if($i != 9)
...
<div class="tbl_head01">
	...
	<tr>
		<td rowspan="2">
			<input type="checkbox" name="chk[]" value="<?php echo $i;?>"<?php if(in_array($set, array('today','whole')) && in_array($row['dan'], array('5','6','10'))) { echo " disabled"; } ?>>
	...
	<tr class="rows">
		<td colspan="3">
			<?php echo get_sorts_conf_select("od_delivery[]", "","id='delivery_{$i}'");?>
	...
	<?php
		if($max['max_idx'] == $od_table) {
			...
			echo "<td class='tar fc_red bold' colspan='2'>총계 : ".number_format($sum['amt']+$sum['del_amt'])."</td>\n";
			echo "</tr>\n";
		}

		echo "<script>document.getElementById('delivery_{$i}').value='$row[delivery]';</script>";
	}
	if($i==0)
...
<div class="local_frm02">
	<?php if(in_array($set, array('1','2','3','4','5'))) { ?>
	<select name="dan">
...
function btn_check(act)
{
    ...
    if(act == "update") // 선택변경
    {
        f.action = './partner_odr_update.php';
        str = "변경";
    }
	else if(act == "dydate") // 선택수정
    {
        f.action = './partner_odr_update_dy.php';
        str = "수정";
    }
    else if(act == "delete") // 선택삭제
    {
        f.action = './partner_odr_delete.php';
        str = "삭제";
    }
    else
        return;
