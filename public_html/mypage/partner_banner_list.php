...
if($mode == 'reset') {
	...
	// 배너
	$sql = "select * from shop_banner where mb_id='$member[id]' ";
	$res = sql_query($sql);
	for($i=0; $row=sql_fetch_array($res); $i++) {
		@unlink(TW_DATA_PATH.'/banner/'.$row['bn_file']);
	}
	sql_query("delete from shop_banner where mb_id='$member[id]'");

	check_banner_copy($member['id']);

	goto_url("./page.php?code=partner_banner_list");
}
...
<a href="javascript:post_reset('page.php?code=partner_banner_list');" class="btn_lsmall bx-red"><i class="fa fa-refresh fa-spin"></i> 본사와 동일하게 설정값 초기화</a>
<a href="page.php?code=partner_banner_form" class="fr btn_lsmall red"><i class="ionicons ion-android-add"></i> 추가하기</a>
EOF;
?>
...
<div class="tbl_head01">
	...
	for($i=0; $row=sql_fetch_array($result); $i++) {
		...
		$s_upd = "<a href='page.php?code=partner_banner_form&w=u&ba_table=$ba_table&page=$page' class=\"btn_small\">수정</a>";
...
function btn_check(act)
{
    ...
    if(act == "update") // 선택수정
    {
        f.action = './partner_banner_list_update.php';
        str = "수정";
    }
    else if(act == "delete") // 선택삭제
    {
        f.action = './partner_banner_list_delete.php';
        str = "삭제";
    }
    else
        return;
