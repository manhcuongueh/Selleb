...
if($mode == 'reset') {
	...
	// 배너
	$sql = "select * from shop_banner_slider where mb_id='$member[id]' ";
	$res = sql_query($sql);
	for($i=0; $row=sql_fetch_array($res); $i++) {
		@unlink(TW_DATA_PATH.'/banner/'.$row['bn_file']);
	}
	sql_query("delete from shop_banner_slider where mb_id='$member[id]'");

	check_main_banner_copy($member['id']);

	goto_url("./page.php?code=partner_slider_list");
}
...
$sql_common = " from shop_banner_slider ";
$sql_search = " where (bn_theme = '{$member['theme']}' or bn_mobile_theme = '{$member['mobile_theme']}') 
				  and mb_id = '$member[id]' ";

$sql_order  = " order by bn_rank desc ";
...
<a href="javascript:post_reset('page.php?code=partner_slider_list');" class="btn_lsmall bx-red"><i class="fa fa-refresh fa-spin"></i> 본사와 동일하게 설정값 초기화</a>
<a href="page.php?code=partner_slider_form" class="fr btn_lsmall red"><i class="ionicons ion-android-add"></i> 추가하기</a>
EOF;
?>
...
<h2>PC스킨(<?php echo $member['theme']; ?>), 모바일스킨(<?php echo $member['mobile_theme']; ?>)</h2>
<div class="local_ov">
...
<div class="tbl_head01">
	...
	for($i=0; $row=sql_fetch_array($result); $i++) {
		...
		$s_upd = "<a href='page.php?code=partner_slider_form&w=u&ba_table=$ba_table&page=$page' class=\"btn_small\">수정</a>";
	...
	<tr class="<?php echo $bg; ?>">
		...
		<td><input type="text" class="frm_input" name="bn_rank[<?php echo $i; ?>]" value="<?php echo $row['bn_rank']; ?>"><p class="mart3"><?php echo $row['bn_device']; ?></p></td>
...
function btn_check(act)
{
    ...
    if(act == "update") // 선택수정
    {
        f.action = './partner_slider_list_update.php';
        str = "수정";
    }
    else if(act == "delete") // 선택삭제
    {
        f.action = './partner_slider_list_delete.php';
        str = "삭제";
    }
    else
        return;
