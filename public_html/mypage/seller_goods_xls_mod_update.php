...
if($_FILES['excelfile']['tmp_name']) {
    ...
    */
	...
	for($i=2; $i<=$data->sheets[0]['numRows']; $i++)
	{
		...
		$daccount	= addslashes(trim(conv_number($data->sheets[0]['cells'][$i][$j++]))); // 공급가격
		$saccount	= addslashes(trim(conv_number($data->sheets[0]['cells'][$i][$j++]))); // 시중가격
		$account	= addslashes(trim(conv_number($data->sheets[0]['cells'][$i][$j++]))); // 판매가격
		...
		$gpoint		= addslashes(trim(conv_number($data->sheets[0]['cells'][$i][$j++]))); // 적립금
		...
		$img_mod	= addslashes(trim(conv_number($data->sheets[0]['cells'][$i][$j++]))); // 이미지등록방식
		...
		$value['shop_state']	= $config['shop_mod_auto']; //자동승인여부
		$value['mb_id']			= $seller['sup_code']; // 판매자ID
		...
		$value['daccount']		= $daccount; // 공급가격
		$value['saccount']		= $saccount; // 시중가격
		$value['account']		= $account; // 판매가격
		...
		// 상품주키값을 불러온다
		$gs = sql_fetch("select index_no from shop_goods where gcode = '$gcode'");
		$gs_id = $gs['index_no'];
...
<div class="btn_confirm">
	<a href="./page.php?code=seller_goods_xls_mod" class="btn_large">확인</a>
</div>
