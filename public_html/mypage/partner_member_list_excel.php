...
if($j_sdate && $j_ddate) {
	$sql_search .= " and (left(reg_time,10) >= '$j_sdate' and left(reg_time,10) <= '$j_ddate')";
}
if($j_sdate && !$j_ddate) {
	$sql_search .= " and (left(reg_time,10) >= '$j_sdate' and left(reg_time,10) <= '$j_sdate')";
}
if(!$j_sdate && $j_ddate) {
	$sql_search .= " and (left(reg_time,10) >= '$j_ddate' and left(reg_time,10) <= '$j_ddate')";
}
if($l_sdate && $l_ddate) {
	$sql_search .= " and (left(today_login,10) >= '$l_sdate' and left(today_login,10) <= '$l_ddate')";
}
if($l_sdate && !$l_ddate) {
	$sql_search .= " and (left(today_login,10) >= '$l_sdate' and left(today_login,10) <= '$l_sdate')";
}
if(!$l_sdate && $l_ddate) {
	$sql_search .= " and (left(today_login,10) >= '$l_ddate' and left(today_login,10) <= '$l_ddate')";
}
