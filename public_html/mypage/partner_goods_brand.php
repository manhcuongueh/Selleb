...
$sql_search = " where br_user_yes = '0' or (br_user_yes = '1' and mb_id = '$member[id]') ";
...
<div class="tbl_head01">
	...
	for($i=0; $row=sql_fetch_array($result); $i++) {
		...
		if($row['mb_id'] == $member['id']) {
			$s_upd = "<a href='./page.php?code=partner_goods_brand_form&w=u&br_id=$br_id$qstr&page=$page' class=\"btn_small\">수정</a>";
			$readonly = "";
			$disabled = "";
...
function fregform_submit(f){
	...
	f.action = "./partner_goods_brand_update.php";
    return true;
}
...
function btn_check(act)
{
    ...
    if(act == "update") // 선택수정
    {
        f.action = './partner_goods_brand_list_update.php';
        str = "수정";
    }
    else if(act == "delete") // 선택삭제
    {
        f.action = './partner_goods_brand_list_delete.php';
        str = "삭제";
    }
    else
        return;
