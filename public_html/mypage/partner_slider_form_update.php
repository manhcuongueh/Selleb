<?php
define('_PURENESS_', true);
include_once("./_common.php");

check_demo();

check_admin_token();

$upl_dir = TW_DATA_PATH."/banner";
$upl = new upload_files($upl_dir);

unset($value);
if($bn_file_del) {
	$upl->del($bn_file_del);
	$value['bn_file'] = '';
}
if($_FILES['bn_file']['name']) {
	$value['bn_file'] = $upl->upload($_FILES['bn_file']);
}

$value['mb_id'] = $member['id'];
$value['bn_device'] = $_POST['bn_device'];
$value['bn_theme'] = $member['theme'];
$value['bn_mobile_theme'] = $member['mobile_theme'];
$value['bn_rank'] = $_POST['bn_rank'];
$value['bn_link'] = $_POST['bn_link'];
$value['bn_target'] = $_POST['bn_target'];
$value['bn_width'] = $_POST['bn_width'];
$value['bn_height'] = $_POST['bn_height'];
$value['bn_bg']	= '#'.rpc($_POST['bn_bg'], "#");
$value['bn_text'] = $_POST['bn_text'];
$value['bn_use'] = $_POST['bn_use'];

if($w == "") {
	insert("shop_banner_slider", $value);
	$ba_table = sql_insert_id();

	$qstr = "code=partner_slider_form&w=u&ba_table=$ba_table";
} else if($w == "u") {
	update("shop_banner_slider", $value," where index_no='$ba_table'");

	$qstr = "code=partner_slider_form&w=u&ba_table=$ba_table&page=$page";
}

goto_url("./page.php?$qstr");
?>